...
const CommunityHoursDashboard: React.FC = () => {
  ...
  const userRole: UserRole | undefined = user?.publicMetadata?.role as UserRole | undefined;
  const userSchoolId: string | undefined = user?.publicMetadata?.schoolId as string | undefined;
  ...
  }, [isLoaded, isSignedIn, userRole, userSchoolId, axiosAuth]);
    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 403) {
          setError(`Permission Denied: ${err.response.data?.message || "You do not have permission to view this data."}`);
        } else if (err.response.status === 401) {
          setError("Authentication expired. Please sign in again.");
          signOut();
        } else {
          setError(`Error: ${err.response.status} - ${err.response.data?.message || err.message || "An unknown error occurred."}`);
        }
      } else if (err.request) {
        setError("Network Error: Could not connect to the backend server. Please check your internet connection or backend server.");
      } else {
        setError(`Failed to fetch users: ${err.message || "An unexpected error occurred."}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  // --- Filtering Logic for School Admin View ---
  ...
  const currentDisplayedUsers = userRole === UserRole.SchoolAdmin ? filteredSchoolAdminUsers : users;

  const totalDisplayedUsers = currentDisplayedUsers.length;
  const totalDisplayedSubmissions = currentDisplayedUsers.reduce(
    (total, u) => total + (u.Submissions?.length || 0), 0
  );
  const totalDisplayedHours = currentDisplayedUsers.reduce(
    (total, u) => total + (u.Submissions ? getTotalHours(u.Submissions) : 0), 0
  );
  ...
  const totalDisplayedSubmittedSubmissions = currentDisplayedUsers.reduce(
    (total, u) =>
      total + (u.Submissions ? u.Submissions.filter((s) => s.status === "SUBMITTED").length : 0), 0
  );
               ...
               <Box>
                ...
                <Typography variant="body1" color="text.secondary">
                  {userRole === UserRole.SchoolAdmin
                    ? `Welcome, ${user?.firstName || user?.emailAddresses?.[0]?.emailAddress || 'School Admin'}! (Managing ${totalDisplayedUsers} students)` // Changed text
          ...
          {userRole === UserRole.SchoolAdmin ? (
            ...
            <Box mt={4} textAlign="center">
              <Typography variant="h5" color="text.primary" gutterBottom>
                Hello, {user?.firstName || user?.emailAddresses?.[0]?.emailAddress || 'Community Member'}!
              </Typography>
              <Typography variant="body1" color="text.secondary" sx={{ mb: 2 }}>
                Your current role is: <span style={{ fontWeight: 'bold' }}>{userRole || 'Student'}</span>.
              </Typography>
              <Alert severity="info" variant="outlined" sx={{ mt: 2, maxWidth: 600, mx: 'auto', borderRadius: 2 }}>
                Content tailored for your role will appear here. For now, this is a placeholder for students or users without specific admin roles.
              </Alert>
            </Box>
          )}
        </Container>
